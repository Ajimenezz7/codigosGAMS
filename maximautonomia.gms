...
Scalar
         ...
         Cd0     'Coeficiente de Resistencia [Adim]'                /0.0209/,
         ...
         AnguloViento   'Ángulo Dirección Viento en [rad] (Viento Descendente)'  /8/;
...
Rest1(j)..                        rho(j) =e= rho0*(1-(lambda*h(j))/(T0))**((g0)/(R*lambda)-1);
...
Rest3(j)..                        0.5*rho(j)*S*V(j)*V(j)*Cl(j) =e= m*g(j)*cos(Gamma_d(j));

*ECUACIÓN DINAMICA DE LA RESISTENCIA

Rest4(j)..                        0.5*rho(j)*S*V(j)*V(j)*(Cd0+K*Cl(j)*Cl(j)) =e=  m*g(j)*sin(Gamma_d(j));

*ECUACIÓN CINEMATICA DE LA ALTURA


Rest5(j)$(jnotlast(j))..          h(j+1)-h(j)=e=0.5*(-V(j)*sin(Gamma_d(j))-V(j+1)*sin(Gamma_d(j+1))- Vw(j)*sin(Theta)- Vw(j+1)*sin(Theta))*step;

*ECUACION CINEMATICA DE LA VELOCIDAD


Rest6(j)$(jnotlast(j))..          x(j+1)-x(j)=e=0.5*(V(j)*cos(Gamma_d(j))+V(j+1)*cos(Gamma_d(j+1))- Vw(j)*cos(Theta)- Vw(j+1)*cos(Theta))*step;
...
Solve MaximumGliderRange using NLP maximizing step;

execute_unload "results.gdx" step

execute 'gdxxrw.exe results.gdx o=results.xls var=step.L'
